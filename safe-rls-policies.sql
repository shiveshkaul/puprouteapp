-- Safe RLS Policies Fix - Only applies policies without recreating tables
...
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'walker_profiles') THEN
...
-- Walker profiles policies (only if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'walker_profiles') THEN
    ...
    CREATE POLICY "Walker profiles are viewable by all" ON public.walker_profiles
      FOR SELECT 
      TO authenticated
      USING (profile_status = 'approved'); -- Use profile_status instead of is_active
...
-- Make sure reference tables are accessible (only if they exist)
DO $$
BEGIN
  ...
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'pet_breeds') THEN
...
-- Create optional indexes if tables exist
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'walker_profiles') THEN
